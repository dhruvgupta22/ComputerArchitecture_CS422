...
/* Hash table for footprint calculation */
typedef struct helem_s {
	ADDRINT block_addr;
	struct helem_s *next;
} helem;

helem *ins_ht[NUM_HT_BUCKETS], *data_ht[NUM_HT_BUCKETS];
...
VOID addFootPrint(helem **ht, ADDRINT addr, UINT32 size)
{
	ADDRINT block = addr >> 5;
	ADDRINT last_block = (addr + size - 1) >> 5;
	UINT32 index;
	helem *prev, *ptr, *ptrnew;
	do
	{
		index = block & (NUM_HT_BUCKETS - 1);
		ptr = ht[index];
		prev = NULL;
		while (ptr != NULL) {
			if (ptr->block_addr >= block) break;
			prev = ptr;
			ptr = ptr->next;
		}
		if ((prev == NULL) && (ptr == NULL)) {
			ptrnew = (helem*)malloc(sizeof(helem));
			ptrnew->next = ptr;
			ptrnew->block_addr = block;
			ht[index] = ptrnew;
		}
		else if ((prev == NULL) && (ptr != NULL)) {
			if (ptr->block_addr != block) {
				ptrnew = (helem*)malloc(sizeof(helem));
                        	ptrnew->next = ptr;
                        	ptrnew->block_addr = block;
				ht[index] = ptrnew;
...
VOID RecordIns(ADDRINT insAddr, UINT32 insSize, UINT32 opCount, UINT32 opRCount, UINT32 opWCount, INT32 maxImm, INT32 minImm)
{
	...
	addFootPrint(ins_ht, insAddr, insSize);
}
	...
	INS_CATEGORY memOp1Category, ADDRINT memOp1Addr, UINT32 memOp1Size, UINT32 readOpCount, UINT32 writeOpCount, ADDRDELTA maxdisp, ADDRDELTA mindisp)
{
	...
	addFootPrint(data_ht, memOp1Addr, memOp1Size);
	...
	INS_CATEGORY memOp2Category, ADDRINT memOp2Addr, UINT32 memOp2Size, UINT32 readOpCount, UINT32 writeOpCount, ADDRDELTA maxdisp, ADDRDELTA mindisp)
{
	...
	addFootPrint(data_ht, memOp2Addr, memOp2Size);
	...
	INS_CATEGORY memOp3Category, ADDRINT memOp3Addr, UINT32 memOp3Size, UINT32 readOpCount, UINT32 writeOpCount, ADDRDELTA maxdisp, ADDRDELTA mindisp)
{
	...
	addFootPrint(data_ht, memOp3Addr, memOp3Size);
	...
	INS_CATEGORY memOp4Category, ADDRINT memOp4Addr, UINT32 memOp4Size, UINT32 readOpCount, UINT32 writeOpCount, ADDRDELTA maxdisp, ADDRDELTA mindisp)
{
	...
	addFootPrint(data_ht, memOp4Addr, memOp4Size);
...
VOID StatDump(void)
{
	...
	*out << "Instruction Register Write Operand Results: " << endl;
        ...
        *out << endl;
	for (UINT32 i = 0; i < NUM_HT_BUCKETS; i++) {
		ptr = ins_ht[i];
		while (ptr != NULL) {
			ptr = ptr->next;
			insFootPrint++;
	...
	for (UINT32 i = 0; i < NUM_HT_BUCKETS; i++) {
        ...
        *out << "Instruction Blocks Accesses : " << insFootPrint << endl;
        *out << "Memory Blocks Accesses : " << memFootPrint << endl;
...
VOID Trace(TRACE trace, VOID *v)
{
	...
	for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
		...
		for (INS ins = BBL_InsHead(bbl); ; ins = INS_Next(ins)) {
			memOps = 0;
			effectiveMemOps = 0;
			effectiveReadMemOps = 0;
			effectiveWriteMemOps = 0;
			ops = INS_OperandCount(ins);
			maxImm = 0;
			minImm = 0xFFFFFFFFLL;
			maxDisp = 0;
			minDisp = 0xFFFFFFFFLL;
...
int main(int argc, char *argv[])
{
	...
	for (UINT32 i = 0; i < NUM_HT_BUCKETS; i++) ins_ht[i] = NULL;
	for (UINT32 i = 0; i < NUM_HT_BUCKETS; i++) data_ht[i] = NULL;
